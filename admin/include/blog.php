            ...
            <div class="card">
              <div class="card-header">
                ...
                <div class="card-tools">
                  <a href="index.php?include=tambah-blog" 
                  class="btn btn-sm btn-info float-right"><i class="fas fa-plus"></i> Tambah  Blog</a>
                </div>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
              ...
              <table class="table table-bordered">
                    ...
                    $sql_jum = "SELECT `b`.`id_blog`, `k`.`kategori_blog`, `b`.`judul`, `b`.`tanggal` 
                                FROM `blog` `b` INNER JOIN `kategori_blog` `k` 
                                ON `b`.`id_kategori_blog` = `k`.`id_kategori_blog`";
                    if (!empty($katakunci_blog)){
                      $sql_jum .= " WHERE `b`.`judul` LIKE '%$katakunci_blog%' 
                                    OR `k`.`kategori_blog` LIKE '%$katakunci_blog%' 
                                    OR `b`.`tanggal` LIKE '%$katakunci_blog%'";
                    } 
                    ...
                    $sql_k = "SELECT `b`.`id_blog`, `k`.`kategori_blog`, `b`.`judul`, `b`.`tanggal` 
                              FROM `blog` `b` INNER JOIN `kategori_blog` `k` 
                              ON `b`.`id_kategori_blog` = `k`.`id_kategori_blog`";
                        if (!empty($katakunci_blog)){
                          $sql_k .=  " WHERE `b`.`judul` LIKE '%$katakunci_blog%' 
                                      OR `k`.`kategori_blog` LIKE '%$katakunci_blog%' 
                                      OR `b`.`tanggal` LIKE '%$katakunci_blog%'";
                        } 
                        $sql_k .= " ORDER BY `b`.`judul`";
                    ...
                    <tr>
                      ...
                      <td align="center">
                        ...
                        <a href="index.php?include=detail-blog&data=<?php echo $id_blog;?>" class="btn btn-xs btn-info" name="detailblog" value="detailblog" title="DetailBLog"><i class="fas fa-eye"></i></a>
                        <a href="javascript:if(confirm('Anda yakin ingin menghapus data <?php echo $judul; ?>?'))
                        window.location.href='index.php?include=blog&aksi=hapus&data=<?php echo $id_blog;?>&notif=hapusblogberhasil'" class="btn btn-xs btn-warning"><i class="fas fa-trash"></i></a>
                      </td>
                    </tr>
                  <?php $posisi++;}?>
                    </tbody>
